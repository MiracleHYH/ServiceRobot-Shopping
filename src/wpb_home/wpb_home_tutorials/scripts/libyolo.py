...
class YoloV5s(Config):
    def __init__(self, weight_path: Union[str, Path] = None) -> None:
        ...
        weight_path: Path = Path(__file__).resolve().parent.parent.joinpath(
            "models", "yolov5m.pt") if weight_path is None else Path(weight_path)
    ...
    def _load(self, weight_path: Union[str, Path]) -> None:
        ...
        self.model = attempt_load(
            weights=weight_path, map_location=self.device)
...
if __name__ == "__main__":
    ...
    while cv2.waitKey(1) != 27 and (re := cap.read())[0]:
        res, img = yolo.inference(img=re[1], if_absolute=False, if_show=True)
        # for pose in res['person']:
        #     cv2.circle(img, (int(pose[0]), int(pose[1])), 10, (0, 0, 255), -1)
        cv2.imshow("test", img)
